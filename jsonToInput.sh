...
cd ${workingDir}
echo "I'm in directory ${workingDir}"
echo $1 > infile.txt
echo "${inputString}" | awk -F';' '{for(i = 1;i <= NF;i++){printf "%s\n", $i}}' | while read LINE; do # >>${workingDir}/file.txt # | while read LINE; do 
	case $LINE in
		...
		;;
                *)
			echo "$LINE Input parameter not known"
                ;;
	esac
done
#echo "export hex=1" >> selector.sh
#echo "export uncert_radius=2" >> selector.sh
...
#yyyymmdd="20${YEAR}${MONTH}${DAY}"
#echo "today = $yyyymmdd"
#yyyymmdd=`./jday ${yyyymmdd} 1`
#DAY=`echo ${yyyymmdd} |cut -c7-8`
#MONTH=`echo ${yyyymmdd} |cut -c5-6`
#YEAR=`echo ${yyyymmdd} |cut -c3-4`
#echo "Day new = $DAY, Date original = ${yyyymmdd}"
if [ $OIL == "NAME" ];then
	OIL_TYPE=${OIL_TYPE//_/ }
        echo $OIL_TYPE
	OIL_TYPE=${OIL_TYPE//§/\&}
...
sed -e "s|SIM_NAME_TMP|$SIM_NAME|g"\
  ...
  if (( $(bc <<< "$START_LON > 0") == 1 ));then
	...
	lineNum=`cat env.sh |grep -n "LON_DEGREE=" | awk 'BEGIN {FS=":"} {print $1}'`
        echo "export LON_DEGREE=$LON_DEGREE" 
        ...
        echo "export LON_MINUTES=$LON_MINUTES" 
...
exit 0
# sim_name=simname;model=MYO1h;wind=ECMWF05;sim_length=0024;day=15;month=10;year=13;hour=08;minutes=00;lat_degree=42;lat_mintes=22.28;lon_degree=10;lon_minutes=55.50;duration=0036;spillrate=00006.80;oil=API;oiltype=17;var_2=1;var_3=0.00;var_10=2.0;var_14=90000;var_19=0.0000033;var_29=0.000008;var_39=150
#####medslik_Input_File.txt.tmp example
#SIM_NAME=SIM_NAME_TMP
#MODEL=MODEL_TMP  # choose between MFS or AFS
#WIND=WIND_TMP     # choose between ECMWF 0.5° or ECMWF 0.25°
#SIM_LENGTH=SIM_LENGTH_TMP    # choose the simulation length (in hours, 4 characters)
#DAY=DAY_TMP  #mettere il giorno successivo a quello in cui si vuole far partire la simulazione
#MONTH=MONTH_TMP
#YEAR=YEAR_TMP
#HOUR=HOUR_TMP #orario in cui si vuole far partire la simulazione
#MINUTES=MINUTES_TIME_TMP
#LAT_DEGREE=LAT_DEGREE_TMP
#LAT_MINUTES=LAT_MINUTES_TMP
#LON_DEGREE=LON_DEGREE_TMP
#LON_MINUTES=LON_MINUTES_TMP
#DURATION=DURATION_TMP             # spill duration, in hours, 4 characters, if the spill is instantaneous type 0000
#SPILL_RATE=SPILL_RATE_TMP        # spill rate in tons/hours,  if the spill is instantaneous write the total tons spilled
#OIL=OIL_TMP
#OIL_TYPE=OIL_TYPE_TMP
#SAT_DATA=SAT_DATA_TMP        # for Point Source choose NO, for slick from satellite data choose YES
###########end of medslik_Input_File.txt.tmp example
